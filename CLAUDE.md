...
11. **A product is a style; each colour it is sold in is its own article.**
    ...
    of moving stock or money through one of them. Two or more colours on a
    plain product make it a style, unless it is flagged `multicolor`: one
    article in several colours (a bicolour frame), which `sync_variants` never
    splits and which can never be a base. Variants are generated by
...
16. **Suspension is the non-payment lever.** `Company.is_active=False` is
    ...
    already holds.
17. **A colour hidden in a code is read, never assumed.** Supplier sheets put
    the colour at the end of the code (`01/1009 C1`, `03/INDAH MATTE BLACK`).
    `services/importer/code_colors.py` detects the rule (separator + up to N
    words, scored by whether tails recur across models), suggests what each
    distinct tail means (remembered answer → numbering series → the shop's
    colours → synonyms) and refuses the batch while any tail is unanswered.
    All it does is fill "Producto base" and "Colores" and add the missing base
    rows; the rest is the ordinary products import, so the family checks and
    rule 11 still apply. Answers live in `color_aliases` per company, the rule
